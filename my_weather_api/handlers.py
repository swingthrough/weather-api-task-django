...
def getWeatherForecast(country_code, for_day, force_api_call=False):
    ...
    if country_code not in COUNTRY_CODES_TO_CAPITAL.keys():
        ret_json_invalid['message'] = f"Value '{country_code}' for query parameter 'country_code' is invalid. Allowed values are {', '.join(COUNTRY_CODES_TO_CAPITAL.keys())}"
        return ret_json_invalid
    
    if validateDateFormat(for_day) == False:
        ret_json_invalid['message'] = f"Value '{for_day}' for query parameter 'date' is invalid. Must be a valid date in format YYYY-MM-DD"
        return ret_json_invalid
    ...
    if query_date < today:
        ret_json_invalid['message'] = f"Value '{for_day}' for query parameter 'date' must be greater or equal to current date: {today.strftime(DATE_FORMAT)}"
        return ret_json_invalid
    
    if query_date > todayPlusTen:
        ret_json_invalid['message'] = f"Value '{for_day}' for query parameter 'date' cannot be larger than 10 days from now - furthest possible date for today is: {todayPlusTen.strftime(DATE_FORMAT)}"
        return ret_json_invalid
